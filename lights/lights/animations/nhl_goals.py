...
from lights.animations.swirl import Swirl
...
colors_per_team = {
    'Ducks': [(252, 76, 2), (185, 151, 91), (193, 198, 200), (0, 0, 0)] * 2,
    'Bruins': [(252, 181, 20), (17, 17, 17)] * 4,
    'Sabres': [(0, 48, 135), (255, 184, 28), (255, 255, 255)] * 2,
    'Flames': [(210, 0, 28), (250, 175, 25), (255, 255, 255)] * 2,
    'Hurricanes': [(206, 17, 38), (255, 255, 255), (164, 169, 173), (0, 0, 0)] * 2,
    'Blackhawks': [(207, 10, 44), (255, 103, 27), (0, 131, 62), (255, 209, 0), (209, 138, 0), (0, 25, 112), (0, 0, 0), (255, 255, 255)],
    'Avalanche': [(111, 38, 61), (35, 97, 146), (162, 170, 173), (0, 0, 0)] * 2,
    'Blue Jackets': [(0, 38, 84), (206, 17, 38), (164, 169, 173)] * 2,
    'Stars': [(0, 200, 71), (143, 143, 140), (17, 17, 17)] * 2,
    'Red Wings': [(206, 17, 38), (255, 255, 255)] * 4,
    'Oilers': [(4, 30, 66), (252, 76, 0)] * 4,
    'Panthers': [(4, 30, 66), (200, 16, 46), (185, 151, 91)] * 2,
    'Kings': [(17, 17, 17), (162, 170, 173), (255, 255, 255)] * 2,
    'Wild': [(175, 35, 36), (2, 73, 48), (237, 170, 0), (226, 214, 181)] * 2,
    'Canadiens': [(175, 30, 45), (25, 33, 104)] * 4,
    'Predators': [(255, 184, 28), (4, 30, 66), (255, 255, 255)] * 2,
    'Devils': [(206, 17, 38), (0, 0, 0), (255, 255, 255)] * 2,
    'Islanders': [(0, 83, 155), (244, 125, 48)] * 4,
    'Rangers': [(0, 56, 168), (206, 17, 38), (255, 255, 255)] * 2,
    'Senators': [(0, 0, 0), (240, 26, 50), (183, 146, 87), (255, 255, 255)] * 2,
    'Flyers': [(247, 73, 2), (0, 0, 0), (255, 255, 255)] * 2,
    'Penguins': [(0, 0, 0), (207, 196, 147), (252, 181, 20), (255, 255, 255)] * 2,
    'Blues': [(0, 47, 135), (252, 181, 20), (4, 30, 66), (255, 255, 255)] * 2,
    'Sharks': [(0, 109, 117), (234, 114, 0), (0, 0, 0)] * 2,
    'Kraken': [(0, 22, 40), (153, 217, 217), (53, 84, 100), (104, 162, 185), (233, 7, 43)] * 2,
    'Lightning': [(0, 40, 104), (255, 255, 255)] * 4,
    'Maple Leafs': [(0, 32, 91), (255, 255, 255)] * 4,
    'Mammoth': [(113, 175, 229), (9, 9, 9), (255, 255, 255)] * 2,
    'Canucks': [(0, 32, 91), (10, 134, 61), (4, 28, 44), (153, 153, 154), (255, 255, 255)] * 2,
    'Golden Knights': [(185, 151, 91), (51, 63, 72), (200, 16, 46), (35, 31, 32), (255, 255, 255)],
    'Capitals': [(4, 30, 66), (200, 16, 46), (255, 255, 255)] * 2,
...
class NHLGoals(BaseAnimation):
    def __init__(self, frameBuf: np.ndarray, *, fps: Optional[int] = 60, speed: float = 2.0,
                 twistSpeed: float = 0.0, goal_light_duration: float = 7.0, play_horns: bool = True):
        super().__init__(frameBuf, fps)

        # start as black and white
        self.colors = np.array([(255, 255, 255), (0, 0, 0)] * 4)

        # start monitoring nhl goals
        self.goal_monitor = NHLGoalMonitor(1.0)
        self.goal_light = GoalLight(frameBuf)
        self.swirl = Swirl(frameBuf, speed=speed, colors=self.colors, twistSpeed=twistSpeed, rainbow=False)
    ...
    def renderNextFrame(self):
        ...
        if self.goal_light_time_remaining > 0:
            ...
            self.goal_light_time_remaining -= 1 / \
                self.fps if self.fps is not None else 1 / 60
            return
        ...
        if scoring_team := self.goal_monitor.pop_goal():
            ...
            if self.play_horns:
                media = self.vlc_instance.media_new(
                    os.path.join(path, songs_per_team[scoring_team]))
                self.vlc_player.set_media(media)
                self.vlc_player.play()

        self.swirl.renderNextFrame()
